...
* @return
*/
var loading = dialog({'id':'loading','padding':'10px','content':'<img src="/Public/Images/loading.gif">'});
function commonJsonAjax(url, param, title) {
    if(!title){
        title = '温馨提示';
    }
    loading.showModal();
    $.ajax({type: "post",url: url,data: param,dataType: 'json',success: function(data){
            loading.close();
            if(!data.status){  
                if(data.buttonValue){
                    //跳转按钮buttonValue,跳转路径redirect
                    dialog({title:title,fixed: true,content: data.info,okValue: data.buttonValue,ok:function(){location.href = data.redirect;},cancelValue: '关闭',cancel: function () {}}).showModal();
                }else{
                    //普通错误
                    dialog({title:title,fixed: true,content: data.info,cancelValue: '关闭',cancel: function () {}}).showModal();
                }
                return false;
            }
            //普通通知
        ...
        error: function () {
            dialog({title:title,fixed: true,content: data.info,cancelValue: '网络错误...',cancel: function () {}}).showModal();
        }
    });
}